...
def _test_simple2(args):
    return test_simple2(*args)


def test_simple2(process_count, config_dict, seed, parallel_on, initial_state=None):
    ...
    if len(result["uct_trees"]) > 0:
        ...
        util.save_pickle(result_no_trees, "../data/test_simple2_result_{}_{}.pkl".format(config_name, process_count))
        if parallel_on: 
            return result_no_trees
    else:
        util.save_pickle(result, "../data/test_simple2_result_{}_{}.pkl".format(config_name, process_count))
    setpoints = np.array(result["rollout_xs"])[:,0:12]
    config_name = os.path.splitext(os.path.basename(config_path))[0]
    np.savetxt("../data/test_simple2_setpoints_{}_{}.csv".format(config_name, process_count), setpoints, delimiter=",")
...
def main():
    ...
    # config_path = util.get_config_path("cast_game2")
    # config_path = util.get_config_path("cast_game2_tree_tuning")
    # config_path = util.get_config_path("cast_game3_exp1")
    # config_path = util.get_config_path("cast_game3_exp3")
    config_path = util.get_config_path("fixed_wing")
    # config_path = util.get_config_path("cast_game2_wobstacle")
    # config_path = util.get_config_path("cast_thermal_observation")


    if only_plot:
        # fns = glob.glob("/home/ben/projects/dots/data/test_simple2_result_cast_game2_*.pkl")
        # fns = glob.glob("../data/test_simple2_result_cast_game2_0.pkl")
        fns = glob.glob("../data/test_simple2_result_fixed_wing_0.pkl")
        print("fns",fns)
        ...
        if parallel_on:
            ...
            results = [x for x in tqdm.tqdm(pool.imap(_test_simple2, args), total=len(args))]
        else:
            results = [_test_simple2(arg) for arg in tqdm.tqdm(args)]
    ...
    for result in results:
        if result is not None:
            plot_result(result)
    plotter.save_figs("../plots/test_simple2.pdf")
    plotter.open_figs("../plots/test_simple2.pdf")
    # plotter.show_figs()
