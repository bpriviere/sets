...
def _test_policy_convergence(args):
    return test_policy_convergence(*args)

def test_policy_convergence(process_count, config_dict, seed, N, parallel_on, initial_state=None):
    ...
    # todo: save data
    util.save_pickle(policy_convergence_result, "../data/test_policy_convergence_result_cast_game2_{}.pkl".format(process_count))
...
def main():
    ...
    if only_plot:
        fns = glob.glob("../data/test_policy_convergence_result_cast_game2_*.pkl")
        print("fns",fns)
        ...
        if parallel_on:
            ...
            results = [x for x in tqdm.tqdm(pool.imap(_test_policy_convergence, args), total=len(args))]
        else:
            results = [_test_policy_convergence(arg) for arg in tqdm.tqdm(args)]
        results = [result for result in results if result is not None]   
        print("total time: {}s".format(timer.time() - start_time))

    plot_policy_convergence_results(results)

    plotter.save_figs("../plots/test_policy_convergence.pdf")
    plotter.open_figs("../plots/test_policy_convergence.pdf")
